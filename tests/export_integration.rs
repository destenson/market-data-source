...
mod json_integration_tests {
    ...
    fn test_json_round_trip() {
        ...
        for (original, parsed) in fixture.ohlc_data.iter().zip(parsed_data.iter()) {
            use rust_decimal::prelude::*;
            assert!(
                (original.open - parsed.open).abs() < Decimal::from_str("0.0000000001").unwrap()
            );
            assert!(
                (original.high - parsed.high).abs() < Decimal::from_str("0.0000000001").unwrap()
            );
            assert!((original.low - parsed.low).abs() < Decimal::from_str("0.0000000001").unwrap());
            assert!(
                (original.close - parsed.close).abs() < Decimal::from_str("0.0000000001").unwrap()
            );
            assert_eq!(original.volume, parsed.volume);
...
mod png_integration_tests {
    ...
    fn test_png_custom_options() {
        use market_data_source::export::{to_png_ohlc_with_builder, ChartBuilder};
        ...
        assert!(
            file_size > 2000,
            "Custom chart too small: {file_size} bytes"
        );
    }
}
...
mod performance_tests {
    ...
    fn test_large_dataset_performance() {
        ...
        println!(
            "Generated {} records for performance test",
            large_data.len()
        );
        ...
        #[cfg(feature = "csv_export")]
        {
            ...
            assert!(
                ...
                "CSV export too slow: {records_per_sec:.0} rec/sec"
            );
        }
        ...
        #[cfg(feature = "json_export")]
        {
            ...
            assert!(
                ...
                "JSON export too slow: {records_per_sec:.0} rec/sec"
            );
        }
        ...
        #[cfg(feature = "png_export")]
        {
            ...
            println!(
                "PNG Chart Performance: {} records in {:?}",
                chart_data.len(),
                duration
            );
            ...
            assert!(
                duration.as_secs() < 30,
                "PNG export took too long: {duration:?}"
            );
        }
    }
}
...
fn test_export_integration_summary() {
    ...
    assert!(
        ...
        "At least one export feature should be enabled for integration tests"
    );
