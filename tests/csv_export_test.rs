...
fn test_stream_csv_export() {
    ...
    let count = generator
        .stream_generate_to_csv_ohlc(100, &file_path)
        .unwrap();
...
fn test_large_dataset_streaming() {
    ...
    let ohlc_count = generator
        .stream_generate_to_csv_ohlc(1000, &ohlc_path)
        .unwrap();
    let tick_count = generator
        .stream_generate_to_csv_ticks(1000, &tick_path)
        .unwrap();
