...
use market_data_source::{
    ConfigBuilder, MarketDataGenerator, MarketRegime, RegimeSchedule, RegimeSegment, TrendDirection,
};
...
fn test_regime_control_end_to_end() {
    ...
    for (i, data) in bull_data.iter().enumerate() {
        if let Some(ref info) = data.schedule_info {
            assert_eq!(
                ...
                "Point {} should be in Bull regime",
                i
            );
        }
    }
    ...
    for (i, data) in sideways_data.iter().enumerate() {
        if let Some(ref info) = data.schedule_info {
            assert_eq!(
                ...
                "Point {} should be in Sideways regime",
                i + 10
            );
        }
    }
    ...
    for (i, data) in bear_data.iter().enumerate() {
        if let Some(ref info) = data.schedule_info {
            assert_eq!(
                ...
                "Point {} should be in Bear regime",
                i + 15
            );
        }
    }
    ...
    for (i, data) in series.iter().enumerate() {
        ...
        assert!(
            data.ohlc.volume.value() > 0,
            "Volume at index {} should be positive",
            i
        );
    }
...
fn test_repeating_regime_schedule() {
    let config = ConfigBuilder::new().seed(456).build().unwrap();
    ...
    let expected_regimes = [
        ...
        MarketRegime::Bear,
        MarketRegime::Bear, // 3-4
        MarketRegime::Bull,
        MarketRegime::Bull,
        MarketRegime::Bull, // 5-7
        MarketRegime::Bear,
        MarketRegime::Bear, // 8-9
        MarketRegime::Bull,
        MarketRegime::Bull, // 10-11
    ];
    ...
    assert!(
        !info.is_complete,
        "Repeating schedule should never be complete"
    );
}
...
fn test_regime_force_override() {
    let config = ConfigBuilder::new().seed(789).build().unwrap();
...
fn test_regime_control_with_transitions() {
    let config = ConfigBuilder::new().seed(999).build().unwrap();
    ...
    for (i, data) in series.iter().enumerate() {
        assert!(
            data.ohlc.is_valid(),
            "OHLC at index {} should be valid during transition",
            i
        );
        ...
        if let Some(ref info) = data.schedule_info {
            ...
            assert!(
                ...
                "Should always have a current regime at index {}",
                i
            );
        }
    }
    ...
    let final_info = generator.regime_control_info().unwrap();
    assert!(
        final_info.is_complete,
        "Schedule should be complete after generating 10 points from 2 segments of 5 each"
    );
}
