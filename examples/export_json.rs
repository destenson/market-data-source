...
use market_data_source::{ConfigBuilder, MarketDataGenerator, TrendDirection};

#[cfg(not(feature = "json_export"))]
fn main() {
    eprintln!(
        "This example requires the 'json_export' feature. Please run with '--features json_export'"
    );
}
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    #[cfg(feature = "json_export")]
    {
        ...
        if let Some(array) = parsed.as_array() {
            println!(
                "   ✓ Successfully parsed JSON file with {} records",
                array.len()
            );
    ...
    println!(
        "   Standard JSON export: {:?} for {} records",
        json_duration,
        large_data.len()
    );
    println!(
        "   JSON Lines export: {:?} for {} records",
        jsonl_duration,
        large_data.len()
    );
