...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let response = client.get(format!("{base_url}/api")).send().await?;
    ...
    let response = client
        .post(format!("{base_url}/api/v1/symbols"))
        .json(&create_request)
        .send()
        .await?;
    ...
    let response = client
        .post(format!("{base_url}/api/v1/generate/BTCUSD"))
    ...
    let response = client
        .get(format!("{base_url}/api/v1/symbols"))
        .send()
        .await?;
    ...
    let response = client
        .get(format!("{base_url}/api/v1/historical/BTCUSD?limit=5"))
        .send()
        .await?;
    ...
    let response = client
        .get(format!("{base_url}/api/v1/algorithms"))
        .send()
        .await?;
    ...
    let response = client
        .get(format!("{base_url}/api/v1/presets"))
        .send()
        .await?;
    let presets: serde_json::Value = response.json().await?;
    let preset_names: Vec<String> = presets["presets"]
        .as_object()
        .unwrap()
        .keys()
        .cloned()
